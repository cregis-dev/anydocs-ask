...
- **RFC 0004 0.4.0-alpha.3 — Widget polish（F10/F11/F12 bundle）** —— 三项 dogfood follow-up 一锅出：(F10) bubble 从「?」文字换成 SVG 对话气泡 icon + 接通 `theme.baseColor` 让客户改主色（host SDK 同时通过 URL param 把主色透传到 iframe，挂成 CSS `--accent` 变量）；(F11) `appendFeedbackBar` 现接 `priorFb` 参数，history restore 时已点过的 turn 会显示 locked + 颜色 tinted 状态（含 read-only 的旧 correction），不再让用户对同一答案重复打分；(F12) 新增 `WidgetInitOptions.docsBaseUrl` —— 客户文档站基础 URL，相对路径 citation 自动拼到该 base 上；未设时相对路径渲染为带 tooltip 的纯文本（避免点开 ask server 自带 reader 的本地 404）。无新增测试 case，既有 widget-server.test.ts 加 7 行断言守住新行为。
