...
> Status: Implemented（A3–A7 已发版；A8 GA flip 进行中）
> 实装状态（2026-06-13）：A3–A6 已于 **0.3.1** 发版、A7 Studio A+ 视图于 **0.4.0-alpha.4** 发版（默认 `aplus.enabled=false`，shadow）；**A8 GA flip** 于 2026-06-13 由 operator 在 hermes-docs 源配置加 `aplus.enabled:true`（pending deploy + 重启 serve 生效）。见 CHANGELOG 0.3.1 / 0.4.0-alpha.4。
> Author: @shawndslee
...
> ✅ **已实装**：A3–A7 已发版（实际 A3–A6 提前并入 0.3.1、A7 在 0.4.0-alpha.4）；A8 GA flip 进行中。

```
alignment PR   (2026-05-24, 本 PR)  Status 升档 + aplus schema + CLI stub     零行为变化   ✅ → 0.3.1
0.4.0-alpha.1                       A3 聚类 pure 模块 + A4 simulator           核心算法     ✅ → 0.3.1
0.4.0-alpha.2                       A5 建议生成 + A6 CLI 接通 pipeline + shadow 可 dogfood  ✅ → 0.3.1
0.4.0-alpha.3                       A7 Studio Feedback tab A+ 视图             可见         ✅ → 0.4.0-alpha.4
0.4.0                               A8 shadow → on (operator flip + threshold) 交付        🔄 进行中（hermes-docs flip 2026-06-13）
```
