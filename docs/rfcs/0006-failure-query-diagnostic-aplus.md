...
```
0.4.0-alpha.0  alignment PR：RFC Draft → Accepted + config schema 留位 +
               CLI feedback diagnose stub                           零行为变化
0.4.0-alpha.1  A3 聚类 pure 模块 + A4 simulator + synthetic fixture  核心算法
0.4.0-alpha.2  A5 建议生成 + A6 CLI 接通 pipeline + shadow 输出      可 dogfood
0.4.0-alpha.3  A7 Studio Feedback tab A+ 视图（依赖 RFC 0002 已就绪） 可见
0.4.0          A8 shadow → on（operator flip + threshold check）     交付
```

绝对日期不预设；里程碑顺序锁定，alpha.0 启动时锚定。`citationSemanticCheck` (RFC 0005) 已经验过该节奏。
