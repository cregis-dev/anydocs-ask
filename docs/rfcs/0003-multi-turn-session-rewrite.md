...
> Status: Implemented（M1–M6 已发版；2026-05-21 方向切换到 B.2 大模型一次消化路径）
> 实装状态（2026-06-13）：M1–M6 已于 **0.2.0**（2026-05-23）作为 early in-scope 发版，默认 `multiTurn.enabled=true`（见 CHANGELOG 0.2.0 / PR #56/#57/#59/#60/#64）。
> Author: @shawndslee
...
> ✅ **已实装**：M1–M6 实际**提前**并入 **0.2.0**（2026-05-23）作为 early in-scope 发版（非下方原计划的 0.4.0-alpha.*），默认 `multiTurn.enabled=true`。下表为原始里程碑规划，保留作历史参照。

```
0.4.0-alpha.0  M1 history-aware retrieve query + 起步联调            基础设施   ✅ → 0.2.0
0.4.0-alpha.1  M2 multi-turn system prompt + 试运行                  核心       ✅ → 0.2.0
0.4.0-alpha.2  M3 + M4 + M5 session 窗口 + 接口字段 + 配置开关        管道       ✅ → 0.2.0
0.4.0          M6 Console Studio 集成 + 整体回归 + 嵌入式联调         交付       ✅ → 0.2.0
```

里程碑顺序锁定；~~绝对日期在 alpha.0 启动时锚定~~ —— **M1–M6 已全部发版（见 CHANGELOG 0.2.0 / PR #56/#57/#59/#60/#64）**。
