...
stable_nav_id(node, file, dfs_path) =
  node.id ?? `nav:${basename(file)}:${dfs_path.join('/')}`
```

- `dfs_path`: 从 navigation 文件根到本节点的子节点序号路径（0-based），不是 page id。例：root.items[0].children[1].children[0] → `0/1/0`。
- 优先用作者显式写的 `id`：anydocs `NavItem.section/folder` 上的 `id` 字段是可选的，写了就用，是最稳定的形态（拖拽不变 id）。
- 未写 `id`：派生 id 是"位置敏感"的——拖拽改顺序会让派生 id 漂移。这破了 PRD §4.6 的"拖拽零 embedding 重算"——但**只破了 nav 投影层，不破 chunks/embedding 层**。投影是毫秒级重算，可接受。
